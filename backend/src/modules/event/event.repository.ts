...
export class EventRepository {
	async createEvent(
		...
		const result = await client.query(
			`INSERT INTO master_event (name, description, notes, date_start, date_end, url_photo, created_at, status) 
	...
	async updateEvent(
		...
		const result = await client.query(
			`UPDATE master_event
				SET name = $1, description = $2, notes = $3, date_start = $4, date_end = $5, url_photo = $6, updated_at = now()
